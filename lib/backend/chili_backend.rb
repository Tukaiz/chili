...
# TODO: NEED TO determine
    def self.create_print_pdf(doc_id, application, env, user_id, unique_id, workflow_path, xml_settings = nil)
      if (Kernel.is_windows? rescue false) #- you may want to implement this method, if you aren't already
        path = "#{workflow_path}\\#{user_id}\\#{unique_id}\\"
      else
        path = "#{workflow_path}/#{user_id}/#{unique_id}/"
      end
      ...
      while task = con.task_status(task_id) and task.finished == 'False'; sleep 1; end

      return false if task.succeeded == 'False'

      url = ChiliService::PreviewResult.parse(task.result).url

      #create the dirs
      if (Kernel.is_windows? rescue false) #- you may want to implement this method, if you aren't already
        r2 = system("mkdir #{workflow_path}\\#{user_id}")
        r3 = system("mkdir #{workflow_path}\\#{user_id}\\#{unique_id}")
      else
        r2 = system("mkdir #{workflow_path}/#{user_id}")
        r3 = system("mkdir #{workflow_path}/#{user_id}/#{unique_id}")
      end
